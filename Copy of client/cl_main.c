...
void CL_WriteConfiguration (void)
{
	...
	Com_sprintf (path, sizeof(path),"baseq2/config.cfg");
	f = fopen (path, "w");
	if (!f)
	{
	...
	fprintf (f, "// generated by zquake2, do not modify\n");
	Key_WriteBindings (f);
	fclose (f);
...
void CL_Frame (int msec)
{
	...
	extratime += msec;
	packet_delta += msec;
	//we use extratime, it is same var, mio
