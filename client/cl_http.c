...
int curlFetch(char url[])
{
  ...
  Com_Printf("HTTP downloading %s... ",url);
  ...
  Com_Printf("HTTP download enabled, url: %s\n", cls.downloadServer);
	strcpy(&buf,cls.downloadServer);
  strcat(&buf,url);
  ...
  if(res != CURLE_OK)
  {
	...
	if (binaryWrite(buf,chunk.memory,chunk.size) != 0)
	{
